...
import { Button, Card, Col, Row } from '../../../../../../../assets/css/styled';
import {
  alignCenter,
  alignFlexStart,
  boxShadowThin,
...
const CardInfo = (props) => {
  ...
  const handleClickButtonCardInfo = (value) => {
    if (value?.action_type === 'webpage') {
      ...
      const tempCardCase = cardCase('webpage1');
      setDialogData(tempCardCase);
      setShowDialogInfo(true);
    } else if (
      datas.appStatus == '135' &&
    ...
    // else if (value?.action_type === 'app_deeplink') {
    //     window.location.assign('https://play.google.com/store/apps/details?id=com.julofinance.juloapp');
    // }
  };
    ...
    cardInfo?.cards.map((item, index) => (
      <Card
        ...
        className={`
                            ...
                            ${cardHome}
                            ${backgroundImage(item?.background_img, 'cover')}
                        `}
        onClick={() =>
          handleClickInfoCard({
