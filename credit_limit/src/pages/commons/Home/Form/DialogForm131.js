...
const DialogForm = ({
  ...
  const { fetchUserStatus, convertDataURLtoFile, handleNotification } =
    useUserContext();
  const [, actions] = useGlobalState();
  const [responseForm131, setResponseForm131] = useState([]);
  const [isScrollFormDialog, setIsScrollFormDialog] = useState(false);

  const [tempImageToUpload, setTempImageToUpload] = useState({});
  ...
  const submitPhoto = async () => {
    ...
    try {
      ...
      if (name === 'selfie' ? response?.length === 1 : response[0].id) {
  ...
  const fetchFormData = async () => {
    try {
      ...
      const listImage = response?.results?.map((item) => {
        return {
      ...
      const setForm131 = FORM_131.map((item) => {
        ...
        const itemIndex = listImage.map((obj) => obj.name).indexOf(item.name);
  ...
  const handleThumbsUp = (obj) => {
    if (obj.value && obj.status == 0) {
      return true;
    } else if (
      obj?.dropdown?.filter((item) => !!item.value && item.status == 0)?.length
    ) {
      return true;
    } else {
      return false;
  ...
  useEffect(() => {
    if (showDialogForm) {
      ...
      if (
        ...
        const tempPhotoIndex = parseInt(localStorage.getItem('photoIndex'));
    ...
    >
      ...
      >
        {responseForm131?.length
          ? responseForm131?.map((item, index, arr) => (
              <div
                key={index}
