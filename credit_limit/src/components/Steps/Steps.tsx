...
const DonutCustom: React.FC<IDonutCustom> = (props) => {
  const { sqSize, strokeWidth, percentage, textInside } = props;
  const radius = sqSize && strokeWidth && (sqSize - strokeWidth) / 2;
  const viewBox = `0 0 ${sqSize} ${sqSize} `;
  const dashArray = radius && radius * Math.PI * 2;
  const dashOffset =
    dashArray && percentage && dashArray - (dashArray * percentage) / 100;
...
export const Steps = (props: Props) => {
  ...
  const handleBack = (value: IHandleBack) => {
    if (value.action === 'redirect') {
      utils.store.set('approveNote', false);
      props?.history?.push({
        pathname: `/${datas.partner}/${value.url}`,
